...
export default function LiquidationPage() {
    ...
    const fetchLiquidations = async (pageNumber = 1) => {
        ...
        try {
            const res = await api.get(`/liquidations?page=${pageNumber}`);

            // ✅ SAME STRUCTURE AS REQUISITION
    ...
    const handleView = async (item) => {
        ...
        try {
            const res = await api.get(`/liquidations/${item.id}`);
        ...
        try {
            ...
            if (type === "approve") {
                if (selected.status === "Pending") {
                    status = "Checked"; // accounting
                } else if (selected.status === "Checked") {
                    status = "Approved"; // HR
                }
            }

            await api.put(`/liquidations/${selected.id}/status`, {
                status,
    ...
    const handleRejectSubmit = () => {
        ...
        rejectAction.run(() => handleAction("reject"));
    };
                ...
                footer={
                    selected?.status === "Checked" && (
                        <div className="w-full flex gap-3">
                            ...
                            <button
                                onClick={() =>
                                    approveAction.run(() => handleAction("approve"))
                                }
                ...
                {loadingDetails ? (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="border rounded-2xl p-5 space-y-2">
                            <p><strong>Reference:</strong> {selected.request_id}</p>
                            <p><strong>Status:</strong> {selected.status}</p>
                        </div>

                        {/* PARTICULARS */}
                        <div className="border rounded-2xl p-5">
                            <h4 className="text-sm font-semibold mb-3">Particulars</h4>
                        ...
                        <div className="border rounded-2xl p-5 text-sm space-y-2">
                            <p>Cash Advance: <strong>{formatCurrency(selected.cash_advance)}</strong></p>
                            <p>Total Expenses: <strong>{formatCurrency(selected.total_expenses)}</strong></p>
                            <p>Returned: <strong>{formatCurrency(selected.amount_returned)}</strong></p>
                            <p>Reimbursement: <strong>{formatCurrency(selected.amount_reimbursement)}</strong></p>
                        </div>
